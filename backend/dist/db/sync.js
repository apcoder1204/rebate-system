...
exports.syncToBackup = syncToBackup;
exports.syncToPrimary = syncToPrimary;
    ...
    try {
        ...
        for (const table of tables) {
            try {
                ...
                const primaryData = await connection_1.primaryPool.query(`SELECT * FROM ${table}`);
                if (primaryData.rows.length === 0) {
                    console.log(`   ⏭️  ${table} is empty, skipping`);
                    continue;
                }
                // Check if table exists in backup
                const tableExists = await connection_1.backupPool.query(`
          SELECT EXISTS (
        ...
        `, [table]);
                ...
                const existingIds = await connection_1.backupPool.query(`SELECT id FROM ${table}`);
                const existingIdSet = new Set(existingIds.rows.map((row) => row.id));
                // Insert new records
                let inserted = 0;
                let skipped = 0;
                for (const row of primaryData.rows) {
                    ...
                    try {
                        await connection_1.backupPool.query(`INSERT INTO ${table} (${columns.join(', ')}) VALUES (${values.join(', ')}) ON CONFLICT DO NOTHING`, placeholders);
                        inserted++;
                    }
                    catch (error) {
...
async function syncToPrimary() {
    ...
    try {
        ...
        for (const table of tables) {
            try {
                console.log(`📦 Syncing ${table}...`);
                const backupData = await connection_1.backupPool.query(`SELECT * FROM ${table}`);
                if (backupData.rows.length === 0) {
                    console.log(`   ⏭️  ${table} is empty, skipping`);
                    continue;
                }
                const existingIds = await connection_1.primaryPool.query(`SELECT id FROM ${table}`);
                const existingIdSet = new Set(existingIds.rows.map((row) => row.id));
                let inserted = 0;
                let skipped = 0;
                for (const row of backupData.rows) {
                    ...
                    const placeholders = columns.map((_, index) => row[columns[index]]);
                    try {
                        await connection_1.primaryPool.query(`INSERT INTO ${table} (${columns.join(', ')}) VALUES (${values.join(', ')}) ON CONFLICT DO NOTHING`, placeholders);
                        inserted++;
                    }
                    catch (error) {
                        console.error(`   ❌ Error inserting row into ${table}:`, error.message);
