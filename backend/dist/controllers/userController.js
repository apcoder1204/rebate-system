...
exports.resetPasswordByEmail = exports.verifyEmailCode = exports.sendEmailVerificationCode = exports.resetPassword = exports.verifyCode = exports.sendVerificationCode = exports.getMyRoleRequest = exports.updateUserRole = exports.deleteUser = exports.reviewRoleRequest = exports.getRoleRequests = exports.listUsers = exports.changePassword = exports.updateProfile = exports.getMe = exports.register = exports.login = void 0;
...
const login = async (req, res) => {
    try {
        ...
        if (!(0, validation_1.isValidEmail)(sanitizedEmail)) {
...
const register = async (req, res) => {
    try {
        ...
        const sanitizedFullName = (0, validation_1.sanitizeString)(full_name);
        ...
        const existingUser = await connection_1.default.query('SELECT id FROM users WHERE email = $1', [sanitizedEmail]);
       ...
       RETURNING id, email, full_name, role, phone, created_date, is_active`, [sanitizedEmail, hashedPassword, sanitizedFullName, formattedPhone, 'user', true, true, true]);
        ...
        if (requested_role && requested_role !== 'user') {
            await connection_1.default.query(`INSERT INTO role_requests (user_id, requested_role, status) 
         VALUES ($1, $2, 'pending')`, [user.id, requested_role]);
        }
        res.status(201).json({
            user: {
                id: user.id,
...
const getMe = async (req, res) => {
    try {
        const result = await connection_1.default.query('SELECT id, email, full_name, role, phone, created_date FROM users WHERE id = $1', [req.user.id]);
        if (result.rows.length === 0) {
...
const updateProfile = async (req, res) => {
    try {
        ...
        if (req.body.full_name !== undefined) {
            ...
            fullName = (0, validation_1.sanitizeString)(req.body.full_name);
        ...
        if (req.body.phone !== undefined) {
            ...
            if (!(0, validation_1.isValidPhone)(trimmedPhone)) {
                return res.status(400).json({ error: 'Invalid phone number format' });
            }
            phone = trimmedPhone;
        }
       ...
       WHERE id = $3
       RETURNING id, email, full_name, role, phone, created_date, updated_date`, [fullName, phone, req.user.id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        res.json({
...
const changePassword = async (req, res) => {
    try {
        ...
        const userResult = await connection_1.default.query('SELECT password_hash FROM users WHERE id = $1', [req.user.id]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        const user = userResult.rows[0];
        const isCurrentValid = await bcryptjs_1.default.compare(current_password, user.password_hash);
        if (!isCurrentValid) {
            return res.status(400).json({ error: 'Current password is incorrect' });
        }
        const isSamePassword = await bcryptjs_1.default.compare(new_password, user.password_hash);
        if (isSamePassword) {
            return res.status(400).json({ error: 'New password must be different from current password' });
        }
        const hashedPassword = await bcryptjs_1.default.hash(new_password, 10);
        await connection_1.default.query('UPDATE users SET password_hash = $1 WHERE id = $2', [hashedPassword, req.user.id]);
...
const getRoleRequests = async (req, res) => {
    try {
      ...
      WHERE rr.status = $1
      ORDER BY rr.requested_date DESC`, [status]);
...
const reviewRoleRequest = async (req, res) => {
    try {
        ...
        const sanitizedComment = comment ? (0, validation_1.sanitizeString)(comment).substring(0, 500) : null;
        // Get the role request
        const requestResult = await connection_1.default.query('SELECT * FROM role_requests WHERE id = $1', [id]);
        ...
        try {
            ...
            // Update role request
            await client.query(`UPDATE role_requests 
         SET status = $1, reviewed_by = $2, reviewed_date = CURRENT_TIMESTAMP, review_comment = $3
         WHERE id = $4`, [newStatus, req.user.id, sanitizedComment, id]);
            // If approved, update user role
            if (action === 'approve') {
                await client.query('UPDATE users SET role = $1 WHERE id = $2', [roleRequest.requested_role, roleRequest.user_id]);
            }
...
const deleteUser = async (req, res) => {
    try {
        ...
        const userResult = await connection_1.default.query('SELECT role FROM users WHERE id = $1', [id]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        ...
        if (targetRole === 'admin') {
            const adminCount = await connection_1.default.query("SELECT COUNT(*) as count FROM users WHERE role = 'admin'");
...
const updateUserRole = async (req, res) => {
    try {
        ...
        // Update user role
        await connection_1.default.query('UPDATE users SET role = $1 WHERE id = $2', [role, id]);
        res.json({
...
const getMyRoleRequest = async (req, res) => {
    try {
        const result = await connection_1.default.query(`SELECT rr.*, 
        reviewer.email as reviewer_email,
        reviewer.full_name as reviewer_name
      FROM role_requests rr
      LEFT JOIN users reviewer ON rr.reviewed_by = reviewer.id
...
exports.getMyRoleRequest = getMyRoleRequest;
...
const sendVerificationCode = async (req, res) => {
    try {
        ...
        if (purpose === 'password_reset') {
            ...
            const userResult = await connection_1.default.query('SELECT id FROM users WHERE phone = $1', [formattedPhone]);
            if (userResult.rows.length === 0) {
...
const verifyCode = async (req, res) => {
    try {
        ...
        const result = await twilioService.verifyCode(phone, code, purpose);
        if (!result.success) {
...
const resetPassword = async (req, res) => {
    try {
        ...
        // Update password
        const hashedPassword = await bcryptjs_1.default.hash(new_password, 10);
        await connection_1.default.query('UPDATE users SET password_hash = $1 WHERE id = $2', [hashedPassword, userResult.rows[0].id]);
...
const sendEmailVerificationCode = async (req, res) => {
    try {
        ...
        // For password reset, check if user exists
        if (purpose === 'password_reset') {
            const userResult = await connection_1.default.query('SELECT id FROM users WHERE email = $1', [sanitizedEmail]);
            if (userResult.rows.length === 0) {
...
const verifyEmailCode = async (req, res) => {
    try {
        ...
        const result = await emailService.verifyEmailCode(sanitizedEmail, code, purpose);
        if (!result.success) {
            return res.status(400).json({ error: result.message || 'Verification failed' });
        }
        res.json({ message: result.message || 'Code verified successfully' });
